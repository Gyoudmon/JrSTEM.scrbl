...
(define mefigure
  (parameterize ([default-uc-block-theme-adjuster actor-colorize])
    (assert (dia-use-case-block #:block-desc captions '#:me #:scale 1.0))))

(define hefigure
  (parameterize ([default-uc-block-theme-adjuster actor-colorize])
    (assert (dia-use-case-block #:block-desc captions '#:he #:scale 1.0))))
  ...
  [#:tree (move-down 0.8 '-===)
   [=> (move-left 1.618)
       ...
       [#:seq
        ...
        [(move-down 2.0 't:goal4 (cons #false t:pin)) => (move-right '#:home '#:/靶子传送界面#screen '#:display)]
        ...
        [(move-down 1.25) => (move-left '#:home '===-)]]]

   ; student's lane
   [=> (move-right 1.618)
       (stamp hefigure 'cb -0.1i)

       [#:tree (move-down 1 's:Modify)
        [=> (move-right-down 0.6 1.0)
            (move-right 0.6 '#:/doc/bullseye.hpp#.s)
            (move-rightward 1 #false '#:save)
            (turn-right-up)
            (move-up 's:Modify '#:/doc/s:PBL.git/)
            (move-to 's:Modify '#:open)]
        [=> (move-right-down 0.6 1.8)
            (move-right 0.6 '#:/doc/bullseye.cpp#.s)
            (move-rightward 1 #false '#:save)
            (turn-right-up)
            (move-to '#:/doc/s:PBL.git/)]]
       
       (move-down 2.0 '#:s#class)
       ...
       [#:tree (move-down 1 'handy? s:pin)
        ...
        [=> (move-down 1.5 's:goal3 (cons "[顺手]" s:pin))
            (move-down 0.65 '.break)
            (move-down 1.25 's:goal4 s:pin)
